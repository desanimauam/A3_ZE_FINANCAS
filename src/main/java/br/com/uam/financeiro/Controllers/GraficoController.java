...
public class GraficoController implements Initializable {
    ...
    private void carregarDados(){
        serieLn.getData().clear();
        Conexao xonn = new Conexao("fin");
        xonn.iniciarConexao();
        String query = "select sum(preco) as soma, concat(year(data_compra),'-',month(data_compra)) as ano from valores group by ano;";
        xonn.setRsTable(query);
        try {
            while(xonn.getRsTable().next()){
                serieLn.getData().add(new XYChart.Data(xonn.getRsTable().getString("ano"), xonn.getRsTable().getInt("soma")));
            }
